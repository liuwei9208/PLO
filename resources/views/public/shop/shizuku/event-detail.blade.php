<x-shizuku-page-layout
    ...
    breadcrumb="すすきのhigh grade health 雫 ＞ トップページ ＞ イベント情報 ＞ {{ $event['title'] }}"
    :assets="[
...
<div class="event-detail-pagination">
    <div class="event-detail-pagination-left">
        @if($prevEvent)
            <a href="{{ route('public.shops.shop.event.detail', ['shop' => $shop, 'id' => $prevEvent['id']]) }}">
    ...
    <div class="event-detail-pagination-center">
        <a href="{{ route('public.shops.shop.event', ['shop' => $shop]) }}">EVENT TOP</a>
    </div>
    <div class="event-detail-pagination-right">
        @if($nextEvent)
            <a href="{{ route('public.shops.shop.event.detail', ['shop' => $shop, 'id' => $nextEvent['id']]) }}">
                {{ $nextEvent['title'] }}&nbsp;
                <svg xmlns="http://www.w3.org/2000/svg" width="8" height="13" viewBox="0 0 8 13" fill="none">
