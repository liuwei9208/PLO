<x-admin-layout>
  ...
  >
    ...
    <div class="mb-6 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      <div class="px-6 py-5">
        <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
          サムネイル（PC、モバイル）<span class="text-error-500">*</span>
        </h3>
      </div>
        ...
        <div class="flex gap-6">
          @for ($i = 1; $i <= 2; $i++)
            @php
              switch ($i) {
                case 1:
                  $saved = $banner['thumbnail_lg'];
                  break;
                case 2:
                  $saved = $banner['thumbnail_sm'];
                  break;
              }
            @endphp
            ...
            for="{{ 'file_' . $i }}"
            >
              <div class="dz-message m-0! flex h-[68px] w-full items-center justify-center rounded-full bg-gray-200 text-gray-700 dark:bg-gray-800 dark:text-gray-400">
            ...
            <p class="mt-1.5 text-xs text-error-500">{{ $errors->first('file_' . $i) }}</p>
            @endif
          @endfor
        </div>
      </div>
    ...
    <!-- thumbnail, URL, Public, Detail URL -->
    <div class="mb-6 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
          ...
          <label for="is_public" class="flex mt-1 cursor-pointer items-center gap-3 text-sm font-medium text-gray-700 select-none dark:text-gray-400">
            <div class="relative">
              <input
                ...
                @if ($banner->is_public) checked @endif
              />
