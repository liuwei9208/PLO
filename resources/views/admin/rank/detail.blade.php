<x-admin-layout>
    <form class="p-4 mx-auto max-w-full md:p-6" method="post" action="{{ url('/admin/rank/' . $rank->id) }}">
        @method('PUT')
        @csrf

        <div x-data="{ pageName: `ランキング編集` }">
        ...
        <div class="mb-6 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="p-4 sm:p-6">
                ...
                <div class="mb-6">
                    ...
                    <input name="rank_name" type="text" value="{{ $rank->name }}"
                ...
                <div>
                    ...
                    <textarea name="description"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ $rank->description }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="flex flex-col px-2 overflow-y-auto modal-content custom-scrollbar">
                ...
                <div class="flex items-center gap-3 mt-6 modal-footer sm:justify-end">
                    ...
                    <form action="{{ url('/admin/rank/' . $rank->id) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
