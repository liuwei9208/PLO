<x-admin-layout>
    <div class="p-4 mx-auto max-w-full md:p-6">
        ...
        <div x-data="{ pageName: `ランキングマスター` }">
            <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90" x-text="pageName"></h2>
                <a href="{{ url('/admin/rank/add') }}"
        ...
        <!-- Search & Limit -->
        <form action="{{ route('admin.rank.index') }}" method="get" id="search_form"
            ...
            <div class="flex p-5 sm:p-6 dark:border-gray-800">
                ...
                <div class="mr-2 hidden lg:block">
                    <div class="relative">
                        ...
                        <input type="text" name="rank" id="search_form_cast" placeholder="ランキング名"
                            value="{{ request()->rank }}"
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center">
                                        <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                            {{ $rank->is_public ? '公開' : '非公開' }}
                                        </p>
                                    </div>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center justify-end">
                                        <a href="{{ url('/admin/rank/' . $rank->id) }}"
        ...
        <div class="mb-10 flex justify-center">
            <div
                ...
                @if ($page > 1)
                    <a href="{{ route('admin.rank.index', array_merge(request()->all(), ['page' => $page - 1])) }}"
                ...
                @for ($i = 1; $i <= $pages; $i++)
                    ...
                    @else
                        <a href="{{ route('admin.rank.index', array_merge(request()->all(), ['page' => $i])) }}"
                            class="flex items-center justify-center w-10 h-10 border-r border-gray-200 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800">
                            {{ $i }}
                        </a>
                    @endif
