<x-admin-layout>
    <form
      ...
      method="post"
      action="{{ url('/admin/appoint/' . $appoint->id) }}"
    >
      @method('PUT')
      @csrf
  
      <div x-data="{ pageName: `パネル編集`}">
        ...
        <div class="p-4 sm:p-6">
          ...
          <div class="mb-6">
            ...
            {{-- <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              本指名値段 <span class="text-error-500">*</span>
            </label>
            <input
              name="repeat_price"
              type="number"
              value="{{ $appoint->repeat_price }}"
            ...
            >
            @if ($errors->has('repeat_price'))
              <p class="mt-1.5 text-xs text-error-500">{{ $errors->first('repeat_price') }}</p>
            @endif --}}
          </div>
          ...
          <!-- Price & Shop -->
           <div class="flex mb-6">
             <div style="margin-right: 24px;">
               <label class="mb-1.5 block text-sm font-medium text-gray-700">
                 パネル指名値段 <span class="text-error-500">*</span>
               </label>
               <div class="flex items-center">
                 <input
                   name="panel_price"
                   type="number"
                   value="{{ $appoint->panel_price }}"
               ...
               @if ($errors->has('panel_price'))
                 <p class="mt-1.5 text-xs text-error-500">{{ $errors->first('panel_price') }}</p>
               @endif
             </div>
             <div>
                  ...
                  @role('admin')
                    @foreach ($shops as $shop)
                      <option
                        value="{{ $shop->id }}"
                        class="text-gray-700"
                        @if ($shop->id == $appoint->shop_id) selected @endif
                      >
                        {{ $shop->name }}
                      </option>
                    @endforeach
