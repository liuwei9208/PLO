<x-admin-layout>
  <form
    ...
    method="post"
    action="{{ url('/admin/ranking/' . $shop->id) }}"
  >
    @method('PUT')
    @csrf
    ...
    @if(session('success'))
      <div class="mb-6 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="p-4 sm:p-6">
          <p style="color:#3B82F6;">{{ session('success') }}</p>
        </div>
      </div>
    @endif
    <!-- Name, Shop, Profile -->
              ...
              >
                ...
                @foreach ($casts as $cast)
                  <option
                    ...
                    @if (
                      (old('rank') && array_key_exists($index, old('rank')) && old('rank')[$index] == $cast->id) ||
                      (
                        isset($rankings[$index]) && $rankings[$index]->cast_id !== null
                        && $rankings[$index]->cast->id === $cast->id
                      )
                    )
                      selected
                    @endif
