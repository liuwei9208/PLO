<x-admin-layout>
    <div class="p-4 mx-auto max-w-full md:p-6">
        ...
        <div x-data="{ pageName: `料金画面管理` }">
            <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90" x-text="pageName"></h2>
                {{-- <a
          href="{{ url('/admin/banner/add') }}"
        ...
        <!-- Search & Limit -->
        <form action="{{ route('admin.banner.index') }}" method="get" id="search_form"
            ...
            <div class="flex p-5 sm:p-6 dark:border-gray-800">
                ...
                <div class="mr-2">
                    <div class="relative z-20 bg-transparent">
                        <select name="public" id="search_form_public"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                全て
                            </option>
            ...
            <!-- Limit -->
            {{-- <div id="diary_index_limit_form" class="p-5 sm:p-6 dark:border-gray-800">
                <div>
                    <div class="relative z-20 bg-transparent">
                        <select name="limit" id="search_form_limit"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            @foreach ([30, 50] as $limit_option)
                                <option value="{{ $limit_option }}"
                                    class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
        ...
        {{-- <p class="px-2 py-2 text-sm text-gray-500 dark:text-gray-400 text-right">
            ...
            {{ $page * $limit > $total ? $total : $page * $limit }}件を表示
        </p> --}}
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center" style="width: 100px;">
                                        @if ($system->header)
                                            <img src="{{ asset('storage/' . $system->header) }}" alt="サムネイル"
                                                class="w-auto ">
                                        @else
                                            <p
                                                class="text-gray-500 text-theme-sm white-space-nowrap dark:text-gray-400">
                                                未設定
                                            </p>
                                        @endif
                                    </div>
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center" style="width: 100px;">
                                        @if ($system->play)
                                            <img src="{{ asset('storage/' . $system->play) }}" alt="サムネイル"
                                                class="w-auto ">
                                        @else
                                            <p
                                                class="text-gray-500 text-theme-sm white-space-nowrap dark:text-gray-400">
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center justify-end">
                                        <a href="{{ url('/admin/system/' . $system->id) }}"
        ...
        {{-- <div class="mb-10 flex justify-center">
            <div
                class="flex align-center rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                @if ($page > 1)
                    <a href="{{ route('admin.cast.index', array_merge(request()->all(), ['page' => $page - 1])) }}"
                ...
                @for ($i = 1; $i <= $pages; $i++)
                    ...
                    @else
                        <a href="{{ route('admin.cast.index', array_merge(request()->all(), ['page' => $i])) }}"
                            class="flex items-center justify-center w-10 h-10 border-r border-gray-200 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800">
                            {{ $i }}
                        </a>
                    @endif
