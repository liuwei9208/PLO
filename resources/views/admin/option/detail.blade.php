<x-admin-layout>
  <form
    ...
    method="post"
    action="{{ url('/admin/option/' . $option->id) }}"
  >
    @method('PUT')
    @csrf

    <div x-data="{ pageName: `オプション編集`}">
      ...
      <div class="p-4 sm:p-6">
        ...
        <div class="mb-6">
          <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
            オプション名 <span class="text-error-500">*</span>
          </label>
          <input
            name="option_name"
            type="text"
          ...
          @if ($errors->has('option_name'))
            <p class="mt-1.5 text-xs text-error-500">{{ $errors->first('option_name') }}</p>
          @endif
        </div>
        ...
        <div class="mb-6">
          ...
          <input
            ...
            type="number"
            value="{{ $option->price }}"
            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full max-w-[250px] rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 text-right"
          ><span class="inline-flex mx-2 text-sm font-medium text-gray-700 dark:text-gray-400">円</span>
          @if ($errors->has('price'))
